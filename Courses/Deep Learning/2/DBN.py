...
class DBN(nn.Module):
    ...
    def train_static(self, train_data,train_labels,num_epochs=50,batch_size=10):
        ...
        for i in range(len(self.rbm_layers)):
            ...
            tensor_x = torch.FloatTensor(tmp) # transform to torch tensors
            tensor_y = torch.FloatTensor(train_labels)
            ...
            self.rbm_layers[i].train(_dataloader , num_epochs,batch_size,i)
            # print(train_data.shape)
            # print(tensor_x.shape[0] )
            # print(tensor_x.view((tensor_x.shape[0] , -1)))
            v = tensor_x.view((tensor_x.shape[0] , -1)) #flatten
