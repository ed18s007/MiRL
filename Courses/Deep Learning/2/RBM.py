...
class RBM(nn.Module):
    ...
    def gaussian_to_visible(self,X):
        '''
        ...
        sample_X_prob = torch.clamp(sample_X_prob, 0.0, 1.0) 
        sample_X_prob[sample_X_prob != sample_X_prob] = 0
        # print("sample_X_prob",sample_X_prob)
    ...
    def train(self,train_dataloader , num_epochs = 50,batch_size=16,stack=1):
        tr_ls, train_std  = [], [] 
        ...
        for epoch in range(1 , num_epochs+1):
            ...
            print("Epoch:{} ,avg_cost = {} ,std_cost = {} ,avg_grad = {} ,std_grad = {}".format(epoch,\
                                                            ...
                                                            torch.mean(grad_),\
                                                            torch.std(grad_)))
            tr_ls.append(torch.mean(cost_))
            train_std.append(torch.std(cost_))
        N = list(np.arange(1,num_epochs+1))
        plt.figure()
        plt.plot(N, tr_ls, label = "train_loss")
        # plt.plot(N, train_std, label = "train_std")
        plt.title("Training Loss and train_std [Epoch {}]".format(epoch))
        plt.xlabel("Epoch #")
        plt.ylabel("Loss")
        # plt.ylim((-1,3))
        plt.legend()
        plt.savefig('wts/LossEpoch-{}.png'.format(stack+1))
        plt.close()
