...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(fc_size,dense)
        self.act1 = nn.ReLU()
        self.fc2 = nn.Linear(dense,7)
        # self.act2 = nn.ReLU()
        # self.fc3 = nn.Linear(dense,7)

    def forward(self,x):
        # print("zdjchbsd",x.shape)
        out = self.CL1_PL1(x)
        out = self.CL2_PL2(out)
        # out = self.CL3_PL3(out)
...
for epoch in range(EPOCHS):
    ...
    torch.save(model.state_dict(), f'second/{epoch}.pt')
    end_time = time.time()
...
plt.xlabel("Epoch #")
plt.ylabel("Loss")
ymin, ymax = min(min(tr_ls),min(vl_ls),min(tes_ls)),max(max(tr_ls),max(vl_ls),max(tes_ls))
plt.ylim((ymin, ymax))
plt.legend()
plt.savefig('second/LossEpoch-{}.png'.format(epoch))
plt.close()
plt.figure()
...
plt.ylabel("Accuracy")
plt.legend()
plt.savefig('second/AccuracyEpoch-{}.png'.format(epoch))
plt.close()
