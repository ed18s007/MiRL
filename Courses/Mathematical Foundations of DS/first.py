...
train_img_ls, test_img_ls, train_cls_ls, test_cls_ls = [], [], [], []

# Images to consider to represent an Image
IMG_NO = 2
...
for fl in dir:
	...
	for fls in a:
		...
		count += 1
		if IMG_NO < count:
			train_img_ls.append(path)
			train_cls_ls.append(fl)
		# print(path)
		if path in train_img_ls:
			final_repr_image += svd_on_top_eig(path, TOP_EIG_VAL)
	final_repr_image = final_repr_image/(10.0 - IMG_NO)
