...
for i in range(len(final_ls)):
    ...
    if int(final_ls[i][2])<=300:
        tmp = y + [final_ls[i][2]/900]
        low_surv.append(tmp)
    elif int(final_ls[i][2])<=450:
        tmp = y + [0.3333 + (final_ls[i][2]-300)/(3*(450-300)) ]
        mid_surv.append(tmp)
    else:
        tmp = y + [0.6667 + (final_ls[i][2]-450)/(3*(1767-450))]
        high_surv.append(tmp)
print(len(low_surv), len(mid_surv), len(high_surv))

def regression_accuracy(y_pred,y_true):
    n1,n2,n3,d1,d2,d3=0,0,0,0,0,0
    for i in range(len(y_true)):
        if y_true[i]<=0.3333:
            d1+=1
            if y_pred[i]<=0.3333:
                n1+=1
        elif y_true[i]<=0.6667:
            d2+=1
            if y_pred[i]>0.3333 and y_pred[i]<=0.6667:
                n2+=1
        elif y_true[i]>0.6667:
            d3+=1
            if y_pred[i]>0.6667:
    ...
    print("Class 1 pred and true ",n1,d1)
    print("Class 1 pred and true ",n2,d2)
    print("Class 1 pred and true ",n3,d3)
    print("Accuracy ",(n1+n2+n3)/(d1+d2+d3))
...
for i in range(k_fold):
    ...
    svm_model_regressor = make_pipeline(SVR(kernel = 'rbf'))
    svm_model_regressor.fit(X_train_normalized, Y_train) 
    predict_test = svm_model_regressor.predict(X_test_normalized)

    regression_accuracy(predict_test,Y_test)
    ...
    mlregressor = make_pipeline(LinearRegression())
    mlregressor.fit(X_train_normalized, Y_train)
    mlr_prediction = mlregressor.predict(X_test_normalized)
    regression_accuracy(mlr_prediction,Y_test)
