...
data_folder_path = "olddatan/training_data/"
labels_folder_path = "olddatan/otest_data/"
# img_save_path = "nff/train_slices/" 
img_save_path = "nff/test_slices/" 
data_folder_path = labels_folder_path

tmp = os.listdir(data_folder_path)
...
# for j in range(0, 3):
    data_fls = os.listdir(data_folder_path + tmp[j])
    # print('Data Files:' ,data_fls)
    for each in tqdm(data_fls):
        data_path = os.path.join(data_folder_path, tmp[j], each) 
        # print(data_path)
        if(each == 'imaging.nii.gz'):
            img_x = sitk.GetArrayFromImage(sitk.ReadImage(data_path))
            norm_img = ct_win(img_x,300, 1200,np.float, [0.0, 1.0])
            ...
            for i in range(num_slices):
                ...
                sitk.WriteImage(pred_slice,img_save_path + 'X/' + str(j+1) + '_' + str(i) + '.nii.gz')
